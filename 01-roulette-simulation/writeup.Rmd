...
```
With these set of parameters, we have a mean earnings of -48.316. 
...
b2 <- b1 %>% group_by(Profit) %>% count() 
colnames(b2) <- c("Profit", "Frequency")
ggplot(data = b2, aes(x = Profit, y = Frequency)) + geom_col(stat = "identity") + labs(title = "Average Earnings Simulation(20 Repetitions)")
...
```
With these set of parameters, the mean earnings is -96.324. We had increased the winning threshold which proves to be problematic for the most part as the player would have to risk more to reach the higher winning threshold(to attain stopping condition) while having only 200$ to play around with. 
...
mean(d2$Profit)
```
As expected, with a higher number of chances, we are able to reach the full 1000$ profit(1000$ being the difference between the winning_threshold and the starting_budget) but we still end up with negative mean earnings of -100.76.
...
Here we find that the average number of plays using the initial given parameters is 208.807
