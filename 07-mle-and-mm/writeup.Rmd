...
curve(
    pgamma(x, coef(fitgammaht)[2], coef(fitgammaht)[1])
  , add = TRUE
  , col = "blue"
  , lwd = 3
)
...
curve(
    dgamma(x, coef(fitgammaht)[2], coef(fitgammaht)[1])
  , add = TRUE
  , col = "blue"
  , lwd = 3
)
...
Maximum Likelihood using the Weibull Distribution seems to be a poor fit for Glycohemoglobin  as significant deviations from the ecdf and the histograms are observed in Glycohemoglobin data while it seems to be a decent fit for the Height of Adult Females data as the CDF and the PDF generated by moments look reasonably similar thought it fails to catch the peaks of the histogram in case of the PDF. The QQ plot using the Weibull Distribution seems to be a decent fit for Height of Adult Females data but a poor fit for the Glycohemoglobin data.
...
(medianweibgh <- qweibull(0.5, scale = coef(fitweibullgh)[1] , shape = coef(fitweibullgh)[2]))
```
...
(medianweibht <- qweibull(0.5, scale = coef(fitweibullht)[2] , shape = coef(fitweibullht)[1]))
```
